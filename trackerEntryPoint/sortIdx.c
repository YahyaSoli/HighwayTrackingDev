...
void sortIdx(const emlrtStack *sp, emxArray_real_T *x, emxArray_int32_T *idx)
{
  ...
  i = idx->size[0];
  idx->size[0] = bLen2;
  emxEnsureCapacity_int32_T(sp, idx, i, &gc_emlrtRTEI);
  idx_data = idx->data;
  for (i = 0; i < bLen2; i++) {
    idx_data[i] = 0;
  }
  st.site = &mp_emlrtRSI;
  emxInit_int32_T(&st, &iwork, &hc_emlrtRTEI);
  i = iwork->size[0];
  iwork->size[0] = bLen2;
  emxEnsureCapacity_int32_T(&st, iwork, i, &hc_emlrtRTEI);
  iwork_data = iwork->data;
  for (i = 0; i < bLen2; i++) {
    iwork_data[i] = 0;
  }
  emxInit_real_T(&st, &xwork, &jc_emlrtRTEI, true);
  i = xwork->size[0];
  xwork->size[0] = bLen2;
  emxEnsureCapacity_real_T(&st, xwork, i, &ic_emlrtRTEI);
  xwork_data = xwork->data;
